...
static std::optional<int64_t> getAddConstant(Value expr, Value base,
  ...
  if (auto addOp = expr.getDefiningOp<arith::AddIOp>()) {
    if (auto constOp = addOp.getLhs().getDefiningOp<arith::ConstantIndexOp>();
        constOp && valsAreEquivalent(addOp.getRhs(), base, loopsIVsMap))
      return constOp.value();
    if (auto constOp = addOp.getRhs().getDefiningOp<arith::ConstantIndexOp>();
  ...
  auto getConstLoopBoundsForIV =
    ...
    if (auto forOp = dyn_cast<scf::ForOp>(parentOp)) {
      if (blockArg != forOp.getInductionVar())
        return std::nullopt;
      pos = 0;
    } else if (auto parOp = dyn_cast<scf::ParallelOp>(parentOp)) {
      auto ivs = parOp.getInductionVars();
      auto it = llvm::find(ivs, blockArg);
      if (it == ivs.end())
        return std::nullopt;
      pos = std::distance(ivs.begin(), it);
    } else {
      return std::nullopt;
    }
...
static bool isLoadOnWrittenVector(memref::LoadOp loadOp, Value writeBase,
  ...
  for (unsigned baseDim = 0; baseDim < baseRank; ++baseDim) {
...
static bool haveNoDataDependenciesExceptSameIndex(
  ...
  auto checkLoadInWalkHasNoIncompatibleDataDeps = [&](Operation *loadOp) {
    ...
    // TODO: once more ops implement MemoryEffectOpInterface, here we should
    // interrupt the walk.
    if (!memOpInterf &&
