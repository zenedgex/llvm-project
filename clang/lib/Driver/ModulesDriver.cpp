...
void GraphWriter<const CompilationGraph *>::writeNodeDefinitions(
  ...
  for (const auto &Node : Nodes) {
    ...
    O << llvm::formatv(
        "\t\"{0}\" [{1}, label=\"{ {2} }\"];\n", DOT::EscapeString(NodeID),
        DTraits.getNodeAttributes(Node, G), DOT::EscapeString(NodeLabel));
  ...
  for (auto &&[ScanInputNode, InputDeps] : ScannedInputsWithDeps) {
    ...
    if (!Inserted) {
      Diags.Report(diag::err_modules_driver_named_module_redefinition)
          ...
          // the scanned source.
          << getFirstInputFilename(*It->second->Job)
          << getFirstInputFilename(*ScanInputNode->Job);
