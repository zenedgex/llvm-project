...
; RUN:     | FileCheck %s --check-prefix=NO-PROFIT
; RUN: opt -disable-output -passes=loop-unroll -debug-only=loop-unroll -pass-remarks=loop-unroll \
...
; RUN:     | FileCheck %s --check-prefix=NO-STRATEGY
; RUN: opt -disable-output -passes=loop-unroll -debug-only=loop-unroll -pass-remarks=loop-unroll \
; RUN:     -pass-remarks-missed=loop-unroll -pass-remarks-analysis=loop-unroll -pragma-unroll-threshold=50 < %s 2>&1 \
; RUN:     | FileCheck %s --check-prefix=SIZE-TOO-LARGE-ORE
; RUN: opt -disable-output -passes=loop-unroll -debug-only=loop-unroll -pass-remarks=loop-unroll \
...
; RUN:     | FileCheck %s --check-prefix=COST-NOT-PROFITABLE
...
; RUN:     | FileCheck %s --check-prefix=AUTO-DISABLED
; RUN: opt -disable-output -passes='loop-unroll<upperbound>' -debug-only=loop-unroll -pass-remarks=loop-unroll \
; RUN:     -pass-remarks-missed=loop-unroll -pass-remarks-analysis=loop-unroll < %s 2>&1 \
; RUN:     | FileCheck %s --check-prefix=UPPER-BOUND-HEURISTIC

; REQUIRES: asserts

; CHECK-LABEL: Loop Unroll: F[full_unroll_simple] Loop %for.body
...
; CHECK-NEXT:   full unroll: size {{[0-9]+}} < threshold {{[0-9]+}}.
...
; PARTIAL-UNROLL-LABEL: Loop Unroll: F[partial_unroll_user_count] Loop %for.body
...
; RUNTIME-UNROLL-LABEL: Loop Unroll: F[runtime_unroll_simple] Loop %for.body
...
; CHECK-LABEL: Loop Unroll: F[pragma_full_unroll] Loop %for.body
; CHECK-NEXT:   Loop Size = 6
...
; CHECK-LABEL: Loop Unroll: F[pragma_unroll_count] Loop %for.body
; CHECK-NEXT:   Loop Size = 6
...
; CHECK-LABEL: Loop Unroll: F[upper_bound_unroll] Loop %for.body
; CHECK-NEXT:   Loop Size = 6
...
; CHECK-NEXT:   Skipping full unroll: cost analysis unavailable.
; CHECK-NEXT:   Trying upper-bound unroll...
; CHECK-NEXT:   Trying loop peeling...
; CHECK-NEXT:   Trying partial unroll...
...
; CHECK-LABEL: Loop Unroll: F[unroll_disabled_metadata] Loop %for.body
...
; CHECK-LABEL: Loop Unroll: F[runtime_small_max_tc] Loop %for.body
; CHECK-NEXT:   Loop Size = 6
; CHECK-NEXT:   Computing unroll count: TripCount=0, MaxTripCount=3, TripMultiple=1
; CHECK-NEXT:   Trying pragma unroll...
; CHECK-NEXT:   Trying full unroll...
...
; PARTIAL-UNROLL-LABEL: Loop Unroll: F[partial_unroll_with_remainder] Loop %for.body
; PARTIAL-UNROLL-NEXT:   Loop Size = 6
...
; CHECK-LABEL: Loop Unroll: F[pragma_enable_small_max_tc] Loop %for.body
; CHECK-NEXT:   Loop Size = 6
; CHECK-NEXT:   Computing unroll count: TripCount=0, MaxTripCount=3, TripMultiple=1
; CHECK-NEXT:   Explicit unroll requested: pragma-enable
; CHECK-NEXT:   Trying pragma unroll...
...
define i32 @pragma_enable_small_max_tc(ptr %A, i32 %n) {
entry:
  %n.clamped = and i32 %n, 3
  br label %for.body
...
; PARTIAL-ALLOW-LABEL: Loop Unroll: F[partial_unroll_cost_analysis] Loop %for.body
...
; PARTIAL-ALLOW-NEXT:   Skipping full unroll: cost analysis unavailable.
...
; CHECK-LABEL: Loop Unroll: F[partial_unroll_no_profit] Loop %for.body
; CHECK-NEXT:   Loop Size = 6
; CHECK-NEXT:   Computing unroll count: TripCount=3, MaxTripCount=0, TripMultiple=3
; CHECK-NEXT:   Trying pragma unroll...
; CHECK-NEXT:   Trying full unroll...
; CHECK-NEXT:   full unroll: size {{[0-9]+}} < threshold {{[0-9]+}}.
; CHECK-NEXT:   Exiting block %for.body: TripCount=3, TripMultiple=0, BreakoutTrip=0
; CHECK-NEXT: COMPLETELY UNROLLING loop %for.body with trip count 3!
; CHECK-NEXT: remark: <unknown>:0:0: completely unrolled loop with 3 iterations

define i32 @partial_unroll_no_profit(ptr %A) {
entry:
  br label %for.body
...
; CHECK-LABEL: Loop Unroll: F[runtime_unroll_disabled_pragma] Loop %for.body
; CHECK-NEXT:   Loop Size = 6
; CHECK-NEXT:   Computing unroll count: TripCount=0, MaxTripCount={{[0-9]+}}, TripMultiple=1
; CHECK-NEXT:   Trying pragma unroll...
; CHECK-NEXT:   Trying full unroll...
...
; AUTO-DISABLED-LABEL: Loop Unroll: F[auto_unroll_not_enabled] Loop %for.body
...
; USER-COUNT-REJECT-LABEL: Loop Unroll: F[user_count_rejected] Loop %for.body
...
; USER-COUNT-REJECT-NEXT:   full unroll: size 42 < threshold 16384.
...
; NO-REMAINDER-LABEL: Loop Unroll: F[pragma_count_rejected] Loop %for.body
...
; NO-REMAINDER-NEXT:   full unroll: size 42 < threshold 16384.
...
; PRAGMA-TC-TOO-LARGE-LABEL: Loop Unroll: F[pragma_full_tc_too_large] Loop %for.body
...
; PRAGMA-TC-TOO-LARGE-NEXT:   full unroll: size 802 < threshold 16384.
...
; COST-ANALYSIS-LABEL: Loop Unroll: F[cost_analysis_detailed] Loop %for.body
...
; COST-ANALYSIS-NEXT:   Not full unroll: cost {{[0-9]+}} >= boosted threshold {{[0-9]+}}.
...
; MAX-COUNT-10-LABEL: Loop Unroll: F[exceeds_max_count] Loop %for.body
...
; MAX-COUNT-10-NEXT:   Not full unroll: trip count 20 exceeds max count 10.
...
; PEEL-LABEL: Loop Unroll: F[explicit_peel] Loop %for.body
...
; CHECK-LABEL: Loop Unroll: F[heuristic_peel] Loop %for.header
...
; THRESHOLDS-ZERO-LABEL: Loop Unroll: F[thresholds_zero] Loop %for.body
...
; NESTED-COST-LABEL: Loop Unroll: F[nested_cost_analysis] Loop %inner.header
; NESTED-COST-NEXT:   Not unrolling: transformation disabled by metadata.
; NESTED-COST-LABEL: Loop Unroll: F[nested_cost_analysis] Loop %outer.header
...
; NESTED-COST-NEXT:   Skipping full unroll: cost analysis unavailable.
...
; CHECK-LABEL: Loop Unroll: F[partial_instead_of_full] Loop %for.body
; CHECK-NEXT:   Loop Size = 6
; CHECK-NEXT:   Computing unroll count: TripCount=5000, MaxTripCount=0, TripMultiple=5000
; CHECK-NEXT:   Explicit unroll requested: pragma-enable
; CHECK-NEXT:   Trying pragma unroll...
; CHECK-NEXT:   Trying full unroll...
; CHECK-NEXT:   Unrolled size {{[0-9]+}} exceeds threshold {{[0-9]+}}; checking for cost benefit.
; CHECK-NEXT: Not analyzing loop cost: trip count too large.
; CHECK-NEXT:   Skipping full unroll: cost analysis unavailable.
; CHECK-NEXT: remark: <unknown>:0:0: unable to fully unroll loop: estimated unrolled size {{[0-9]+}} exceeds threshold {{[0-9]+}}
; CHECK-NEXT:   Trying upper-bound unroll...
; CHECK-NEXT:   Trying loop peeling...
; CHECK-NEXT:   Trying partial unroll...
; CHECK-NEXT:   Unrolled size exceeds threshold; reducing count from {{[0-9]+}} to {{[0-9]+}}.
; CHECK-NEXT:   Partially unrolling with count: 2500
; CHECK-NEXT:   Exiting block %for.body: TripCount=5000, TripMultiple=0, BreakoutTrip=0
; CHECK-NEXT: UNROLLING loop %for.body by 2500!
; CHECK-NEXT: remark: <unknown>:0:0: unrolled loop by a factor of 2500
...
; NO-PROFIT-LABEL: Loop Unroll: F[no_profitable_count] Loop %for.body
...
; NO-PROFIT-NEXT:   Skipping full unroll: cost analysis unavailable.
...
; UNROLL-AS-DIRECTED-FAIL-LABEL: Loop Unroll: F[unroll_as_directed_fail] Loop %for.body
...
; UNROLL-AS-DIRECTED-FAIL-NEXT:   Skipping full unroll: cost analysis unavailable.
...
; USER-COUNT-EXCEED-LABEL: Loop Unroll: F[user_count_exceed]
...
; USER-COUNT-EXCEED-NEXT:   full unroll: size 50 < threshold 16384.
...
; CHECK-LABEL: Loop Unroll: F[inline_prevents_unroll]
...
; NO-REMAINDER-LABEL: Loop Unroll: F[small_max_trip_count]
...
; NO-STRATEGY-LABEL: Loop Unroll: F[no_strategy_pragma]
...
; NO-STRATEGY-NEXT:   Runtime unrolling with count: 0
...
; MAX-COUNT-10-LABEL: Loop Unroll: F[tc_exceeds_max_ore] Loop %for.body
; MAX-COUNT-10-NEXT:   Loop Size = 6
; MAX-COUNT-10-NEXT:   Computing unroll count: TripCount=20, MaxTripCount=0, TripMultiple=20
; MAX-COUNT-10-NEXT:   Explicit unroll requested: pragma-enable
; MAX-COUNT-10-NEXT:   Trying pragma unroll...
; MAX-COUNT-10-NEXT:   Trying full unroll...
; MAX-COUNT-10-NEXT:   Not full unroll: trip count 20 exceeds max count 10.
...
; SIZE-TOO-LARGE-ORE-LABEL: Loop Unroll: F[size_too_large_ore] Loop %for.body
; SIZE-TOO-LARGE-ORE-NEXT:   Loop Size = 6
; SIZE-TOO-LARGE-ORE-NEXT:   Computing unroll count: TripCount=50, MaxTripCount=0, TripMultiple=50
; SIZE-TOO-LARGE-ORE-NEXT:   Explicit unroll requested: pragma-enable
; SIZE-TOO-LARGE-ORE-NEXT:   Trying pragma unroll...
; SIZE-TOO-LARGE-ORE-NEXT:   Trying full unroll...
; SIZE-TOO-LARGE-ORE-NEXT:   Unrolled size {{[0-9]+}} exceeds threshold {{[0-9]+}}; checking for cost benefit.
; SIZE-TOO-LARGE-ORE-NEXT: Not analyzing loop cost: trip count too large.
; SIZE-TOO-LARGE-ORE-NEXT:   Skipping full unroll: cost analysis unavailable.
; SIZE-TOO-LARGE-ORE-NEXT: remark: <unknown>:0:0: unable to fully unroll loop: estimated unrolled size {{[0-9]+}} exceeds threshold {{[0-9]+}}
; SIZE-TOO-LARGE-ORE-NEXT:   Trying upper-bound unroll...
; SIZE-TOO-LARGE-ORE-NEXT:   Trying loop peeling...
; SIZE-TOO-LARGE-ORE-NEXT:   Trying partial unroll...
; SIZE-TOO-LARGE-ORE-NEXT:   Unrolled size exceeds threshold; reducing count from {{[0-9]+}} to {{[0-9]+}}.
; SIZE-TOO-LARGE-ORE-NEXT:   Partially unrolling with count: 25
; SIZE-TOO-LARGE-ORE-NEXT:   Exiting block %for.body: TripCount=50, TripMultiple=0, BreakoutTrip=0
; SIZE-TOO-LARGE-ORE-NEXT: UNROLLING loop %for.body by 25!
; SIZE-TOO-LARGE-ORE-NEXT: remark: <unknown>:0:0: unrolled loop by a factor of 25

define i32 @size_too_large_ore(ptr %A) {
entry:
  br label %for.body
...
; CHECK-LABEL: Loop Unroll: F[caller_with_inlined_loop] Loop %for.body.i
...
; CHECK-NEXT:   Trying pragma unroll...
; CHECK-NEXT:   Trying full unroll...
; CHECK-NEXT:   full unroll: size 18 < threshold 16384.
...
; COST-NOT-PROFITABLE-LABEL: Loop Unroll: F[cost_not_profitable] Loop %for.body
...
; COST-NOT-PROFITABLE-NEXT:   Not full unroll: cost {{[0-9]+}} >= boosted threshold {{[0-9]+}}.
; COST-NOT-PROFITABLE-NEXT: remark: <unknown>:0:0: unable to fully unroll loop: estimated unrolled cost {{[0-9]+}} exceeds boosted threshold {{[0-9]+}}
...
; UPPER-BOUND-HEURISTIC-LABEL: Loop Unroll: F[upper_bound_heuristic] Loop %for.body
...
; UPPER-BOUND-HEURISTIC-NEXT:   upper-bound unrolling: size {{[0-9]+}} < threshold {{[0-9]+}}.
...
; CHECK-LABEL: Loop Unroll: F[multi_exit_known_and_unknown] Loop %for.header
; CHECK-NEXT:   Loop Size = 8
...
for.header:
  %i = phi i32 [ 0, %entry ], [ %inc, %for.latch ]
  %sum = phi i32 [ 0, %entry ], [ %add, %for.latch ]
  ; This exit has known trip count = 4
...
; PARTIAL-UNROLL-LABEL: Loop Unroll: F[header_exit_with_remainder] Loop %while.header
...
; CHECK-LABEL: Loop Unroll: F[switch_exit_full_unroll_bug] Loop %loop
; CHECK-NEXT:   Loop Size = 6
; CHECK-NEXT:   Computing unroll count: TripCount=5, MaxTripCount=0, TripMultiple=5
; CHECK-NEXT:   Trying pragma unroll...
; CHECK-NEXT:   Trying full unroll...
...
; PARTIAL-UNROLL-LABEL: Loop Unroll: F[switch_exit_partial_remainder_bug] Loop %loop
; PARTIAL-UNROLL-NEXT:   Loop Size = 6
; PARTIAL-UNROLL-NEXT:   Computing unroll count: TripCount=11, MaxTripCount=0, TripMultiple=11
; PARTIAL-UNROLL-NEXT:   Explicit unroll requested: user-count
; PARTIAL-UNROLL-NEXT:   Trying pragma unroll...
