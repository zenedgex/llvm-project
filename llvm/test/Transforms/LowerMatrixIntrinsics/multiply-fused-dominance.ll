...
define void @multiply_phi_dominates_matmul(ptr noalias %A, ptr noalias %B, ptr noalias %C, i64 %n) {
; CHECK-LABEL: @multiply_phi_dominates_matmul(
; CHECK-NEXT:  entry:
...
loop:
  ...
  %b = load <4 x double>, ptr %B, align 8
  %c = call <4 x double> @llvm.matrix.multiply(<4 x double> %a, <4 x double> %b, i32 2, i32 2, i32 2)
  %offset = mul i64 %iv, 32
  %dst = getelementptr i8, ptr %C, i64 %offset
  store <4 x double> %c, ptr %dst, align 8
