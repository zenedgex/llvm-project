...
linearFunctionTestReplace(Loop *L, BasicBlock *ExitingBB,
  ...
  if (auto *BO = dyn_cast<BinaryOperator>(IncVar)) {
    ...
    // properties from the original loop exit condition. If the original comparison
    // was signed and SCEV computed an exit count, then NSW is safe. Similarly,
    // if it was unsigned, NUW is safe. This is because SCEV's exit count analysis
    // proves the loop exits before overflow occurs for that signedness.
    bool KeepNSW = false;
    bool KeepNUW = false;
