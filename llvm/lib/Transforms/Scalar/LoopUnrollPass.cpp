...
bool llvm::computeUnrollCount(
  ...
  if (PP.PeelCount) {
    ...
    LLVM_DEBUG(dbgs() << " Using explicit peel count: " << PP.PeelCount
                      << "\n");
    UP.Count = 1;
    UP.Runtime = false;
    return true;
  ...
  // 1st priority is unroll count set by "unroll-count" option.
  // 2nd priority is unroll count set by pragma.
  LLVM_DEBUG(dbgs() << " Trying pragma unroll...\n");
  ...
  // of some exit test.
  LLVM_DEBUG(dbgs() << " Trying full unroll...\n");
  UP.Count = 0;
  if (TripCount) {
    UP.Count = TripCount;
    if (auto UnrollFactor =
            shouldFullUnroll(L, TTI, DT, SE, EphValues, TripCount, UCE, UP, ORE,
                             ExplicitUnroll)) {
      UP.Count = *UnrollFactor;
      UseUpperBound = false;
      return ExplicitUnroll;
  ...
  LLVM_DEBUG(dbgs() << " Trying upper-bound unroll...\n");
  ...
  LLVM_DEBUG(dbgs() << " Trying loop peeling...\n");
  computePeelCount(L, LoopSize, PP, TripCount, DT, SE, TTI, AC, UP.Threshold);
  if (PP.PeelCount) {
    LLVM_DEBUG(dbgs() << " Peeling with count: " << PP.PeelCount << "\n");
    UP.Runtime = false;
    UP.Count = 1;
    return ExplicitUnroll;
  }
  ...
  LLVM_DEBUG(dbgs() << " Trying partial unroll...\n");
  if (auto UnrollFactor = shouldPartialUnroll(LoopSize, TripCount, UCE, UP)) {
    ...
    if (PragmaFullUnroll && TripCount && UP.Count && UP.Count != TripCount) {
      LLVM_DEBUG(dbgs() << " Partial unroll instead of full: unrolled size "
                           "too large. Unrolling "
    ...
    if (UP.PartialThreshold != NoThreshold) {
      if (UP.Count == 0) {
        if (PragmaEnableUnroll) {
          LLVM_DEBUG(dbgs() << " Not unrolling as directed: unrolled size too "
                            << "large.\n");
          ORE->emit([&]() {
            return OptimizationRemarkMissed(DEBUG_TYPE,
  ...
  if (PragmaFullUnroll) {
    LLVM_DEBUG(dbgs() << " Not fully unrolling as directed: loop has runtime "
                      << "trip count.\n");
    ORE->emit([&]() {
  ...
  LLVM_DEBUG(dbgs() << " Trying runtime unroll...\n");
  // Don't unroll a runtime trip count loop when it is disabled.
  if (hasRuntimeUnrollDisablePragma(L)) {
    LLVM_DEBUG(dbgs() << " Not runtime unrolling: disabled by pragma.\n");
    UP.Count = 0;
    return false;
  }
  ...
  if (MaxTripCount && !UP.Force && MaxTripCount < UP.MaxUpperBound) {
    LLVM_DEBUG(dbgs() << " Not runtime unrolling: max trip count "
                      << MaxTripCount << " is small (< " << UP.MaxUpperBound
                      << ") and not forced.\n");
